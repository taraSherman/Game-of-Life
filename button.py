...
class Button:
    def __init__(self, surface, x, y, width, height, state='', id='', function=0, color=(255, 255, 255), hover_color=(255, 255, 255), border=True, border_width=2, border_color=(0, 0, 0), text='', font_name='arial', text_size=18, text_color=(0, 0, 0), bold_text=False):
        self.type = 'button'
        self.x = x
        self.y = y
        self.pos = vec(x, y)
    ...
    def show_text(self):
        ...
        size = text.get_size()
        x, y = self.width // 2 - \
            (size[0] // 2), self.height // 2 - (size[1] // 2)
        pos = vec(x, y)
